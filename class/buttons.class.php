...
class nsButtons
{
    ...
    public function Add($File, $Alt = false, $Query = false, $Cnf = false, $Target = false)
    {
        if (!$File) {
            return false;
        }
        ...
        if ($Cnf) {
            $Cnf = "onclick=\"return confirm('" . $Cnf . "');\"";
        }
    ...
    public function AddJava($File, $Alt, $Java)
    {
        if (!$File) {
            return false;
        }
    ...
    {
        if ($this->Row == 0) {
            $this->RowSize = count($this->Buttons);
        }
        if ($this->RowSize == 0) {
            return false;
        }
    ...
    public function DoAdd($Button): void
    {
        $this->Buttons[] = $Button;
    }
