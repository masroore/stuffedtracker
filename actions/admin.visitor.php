...
if (!$Visitor->COMPANY_ID) {
    $Visitor->COMPANY_ID = $CpId;
}
if (ValidVar($Visitor->NAME)) {
    $TableCaption = $Visitor->NAME;
}
...
if (!$nsUser->ADMIN) {
    ...
    $WhereArr = [];
    if (!ValidVar($WhereIpIn) && !ValidVar($WhereIpTempl)) {
...
function UpdateVisitor($Id, $EditArr): void
{
...
function SetVisWatch($VisId, $UserId): void
{
    global $Db;
...
function RemoveVisWatch($VisId, $UserId): void
{
    global $Db;
...
function RemoveIp($IpId, $VisId): void
{
    global $Db;
