...
if (ValidVar($_GET['EditId'])) {
    $EditId = $_GP['EditId'];
}
if (ValidVar($_POST['EditId'])) {
    $EditId = $_GP['EditId'];
}
if (ValidVar($_POST['EditArr'])) {
    $EditArr = $_GP['EditArr'];
}
if (ValidVar($_GET['DeleteId'])) {
    $DeleteId = $_GP['DeleteId'];
}
$ParentId = (ValidId($_GP['ParentId'])) ? $_GP['ParentId'] : 0;
if (ValidId($_GP['MoveId'])) {
    $MoveId = $_GP['MoveId'];
}
if (ValidId($_GP['MoveTo'])) {
    $MoveTo = $_GP['MoveTo'];
}
if (ValidVar($_GET['SortId'])) {
    $SortId = $_GP['SortId'];
}
if (ValidVar($_GET['SortTo'])) {
    $SortTo = $_GP['SortTo'];
}
...
function CreateCampaign(&$Arr, $ParentId = false): void
{
...
function UpdateCampaign($Id, &$Arr): void
{
...
function MoveCampaign($MoveId, $MoveTo): void
{
    global $Db, $nsProduct;
...
function SetGrpWatch($GrpId, $UserId): void
{
    global $Db;
...
function RemoveGrpWatch($GrpId, $UserId): void
{
    global $Db;
