...
if ($CodeType != 1 && $CodeType != 2 && $CodeType != 3 && $CodeType != 4) {
    $CodeType = 1;
}
if ($CodePlace != 1 && $CodePlace != 2 && $CodePlace != 3) {
    $CodePlace = 1;
}
...
if (ValidId($SiteId)) {
    ...
    if (count($ActionsArr) == 0) {
        ...
        if (!$ActionId) {
            $ActionId = $ActionsArr[0]->ID;
        }
    }
}

if ($nsProduct->LICENSE == 3 && !$AllowPhp && $CodeType == 4) {
    $CodeType == 1;
}
...
if (ValidId($SiteId)) {
    $ResultCode = '';
    if (!$nsProduct->WHITE && $CodeType != 4) {
        $ResultCode .= '<!-- Start of Stuffed Tracker ' . $nsProduct->VERSION . ' code for http://' . $Site->HOST . " -->\n";
    }
    if (!$nsProduct->WHITE && $CodeType == 4) {
    ...
    if ($CodePlace == 2) {
        if ($CodeType == 1) {
            if (!MOD_R) {
                $SalePath = str_replace('&', "'+nsAmp+'", $SalePath);
            }
    ...
    if ($CodePlace == 3) {
        if ($CodeType == 1) {
            if (!MOD_R) {
                $ActionPath = str_replace('&', "'+nsAmp+'", $ActionPath);
            }
    ...
    if ($CodePlace == 1) {
        $TopCodeHelp = $Lang['CodeHelpVis'];
    }
    if ($CodePlace == 2) {
        $TopCodeHelp = $Lang['CodeHelpSale'];
    }
    if ($CodePlace == 3) {
        $TopCodeHelp = $Lang['CodeHelpAction'];
    }

    if ($CodeType == 1) {
        $TopCodeHelp .= $Lang['CodeHelpJs'];
        if ($CodePlace == 2) {
            $CodeComment = $Lang['SaleCommentJs'];
        }
    }
    if ($CodeType == 3) {
        $TopCodeHelp .= $Lang['CodeHelpHtml'];
        if ($CodePlace == 2) {
            $CodeComment = $Lang['SaleCommentHtml'];
        }
    }
    if ($CodeType == 4) {
        $TopCodeHelp .= $Lang['CodeHelpPhp'];
        if ($CodePlace == 2) {
            $CodeComment = $Lang['SaleCommentPhp'];
        }
    }
}
