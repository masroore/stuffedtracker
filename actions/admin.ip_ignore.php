...
function AddNewIp($IP, $Descr = false): void
{
    global $Db, $nsProduct;
    if (!$Descr) {
        $Descr = '';
    ...
    if ($CheckId) {
        $Query = 'UPDATE ' . PFX . "_tracker_ip SET IGNORED='1', DESCRIPTION=? WHERE ID = $CheckId";
    } else {
        $Query = 'INSERT INTO ' . PFX . "_tracker_ip (IP, IGNORED, DESCRIPTION) VALUES ('$IP', '1', ?)";
    }
