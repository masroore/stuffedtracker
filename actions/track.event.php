...
if (NS_TRACK_MISC::ValidVar($_GP['ref'])) {
    $Ref = $_GP['ref'];
} else {
...
unset($CurGet['eid'], $CurGet['sc'], $CurGet['action'], $CurGet['rurl'], $CurGet['itm'], $CurGet['RequestPath'], $CurGet['RequestParam'], $CurGet['ref'], $CurGet['CheckCookieId']);
...
if (!$Db->ID) {
    ...
    if ($rurl) {
        NS_TRACK_MISC::Redir(urldecode($rurl));
    }
    @clearstatcache();
    $f = @fopen(self . '/store/redir_action.nodb', 'rb');
    if (!$f) {
    ...
    if (!NS_TRACK_MISC::ValidArr($DataArr)) {
        NS_TRACK_MISC::Redir($_SERVER['HTTP_REFERER']);
    }
    $Action = false;
...
$CompanyId = $Site->COMPANY_ID; if ($Event->ACTIVE != 1) {
    $Skip = true;
}
...
if (!$CodeAction) {
    if (NS_TRACK_ACTION::CompareTemplate($Qr, $Event->QUERY)) {
        ...
        if ($PageId > 0) {
            ...
            if (!$KeepNoRef && $RefSet == 0 && !NS_TRACK_MISC::ValidId($_COOKIE['ns_log'])) {
                $Skip = true;
            }
            if ($Skip && isset($rurl)) {
                NS_TRACK_MISC::Redir($rurl);
