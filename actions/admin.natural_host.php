...
if (isset($_GP['UpdateGrp'])) {
    $UpdateGrp = $_GP['UpdateGrp'];
}
...
if (!$nsUser->DEMO) {
    ...
    if ($Mode == 'Host' && ValidId($EditId) && ValidArr($EditArr)) {
        UpdateHost($EditId, $EditArr);
    }
    if ($Mode == 'Host' && ValidVar($EditId) == 'new' && ValidArr($EditArr)) {
        CreateHost($EditArr);
    }
    if ($Mode == 'Host' && ValidId($DeleteId)) {
        DeleteHost($DeleteId);
    }
...
if ($Mode == 'List' && !ValidId($EditId) && $EditId != 'new') {
    ...
    if ($RegSearchGrp) {
        ...
        if (!$RegGrp->REGULAR_EXPRESSION && !$RegGrp->REGULAR_EXPRESSION2) {
            $RegSearchGrp = false;
        }
    ...
    while ($Row = $Sql->Row()) {
        if ($RegSearchGrp || $RegCheck) {
            //echo $R1." ".$Row->HOST."<br>";
            if ($R1 && !preg_match('/' . $R1 . '/i', $Row->HOST)) {
                continue;
            }
            if ($R2 && preg_match('/' . $R2 . '/i', $Row->HOST)) {
                continue;
            }
        }
        if (!$Row->KEY_VAR && $Row->GKEY_VAR) {
            $Row->KEY_VAR = $Row->GKEY_VAR;
        }
        if (!$Row->BAN && $Row->GBAN) {
            $Row->BAN = $Row->GBAN;
        }
...
function MoveHostsToGrp($GrpId, $Arr): void
{
    global $Db, $nsProduct;
    foreach ($Arr as $Key => $Value) {
        if ($Value != 1) {
...
function MoveHostsFromGrp($Arr): void
{
    ...
    while ($Row = $Sql->Row()) {
        ...
        foreach ($HostList as $HostId => $SubRow) {
            if ($Row->REGULAR_EXPRESSION
                && !@preg_match('/' . $Row->REGULAR_EXPRESSION . '/i', $SubRow->HOST)) {
                continue;
            }
            if ($Row->REGULAR_EXPRESSION2
                && @preg_match('/' . $Row->REGULAR_EXPRESSION2 . '/i', $SubRow->HOST)) {
                continue;
            }
