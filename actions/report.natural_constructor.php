...
$Limit = (ValidVar($_GP['Limit'])) ? (int) ($_GP['Limit']) : 100;
...
if ($ShowAll != 0 && $ShowAll != 1 && $ShowAll != 2) {
    $ShowAll = 1;
}
...
if ($SaveQ && defined('DEBUG') && DEBUG) {
    $Report->LoggerFunc = 'QueriesLog';
}
...
if ($GroupBy && $GroupBy != $CurrentGroupBy) {
    $Inx = count($WhereArr);
    if ($Inx > 0 && (!isset($WhereArr[$Inx - 1]['Id']) || $WhereArr[$Inx - 1]['Id'] == '')) {
        --$Inx;
    }
...
if (!$FormUsed && ($ViewDate || $StartDate || $EndDate || $Filter)) {
    $ShowFilter = 1;
}
...
for ($i = 0; $i < count($GroupOrder); ++$i) {
    ...
    if (!ValidVar($WhereForm[$GrpKey])) {
        $GroupByForm[$GrpKey] = $GroupOrder[$i]['Name'];
    }
}
...
if (!$GroupBy && !$NoGroupBy) {
    if (!$CurrentOrder || $CurrentOrder == (count($GroupOrder) - 1)) {
        $CurrentOrder = 0;
    } else {
        ++$CurrentOrder;
    }
    ...
    if ($GroupBy) {
        $nsProduct->Redir('natural_constructor', $Get . "&GroupBy=$GroupBy&Redir=1", 'report');
    }
}
...
if ($PrevReport && $nsUser->Columns->CONVERSIONS) {
    if ($ShowAll == 1) {
        $PrevReport->NoRef = true;
    }
    if ($ShowAll == 2) {
        $PrevReport->OnlyNoRef = true;
    }
...
if (ValidVar($Report->StatArr)) {
    ...
    foreach ($Report->StatArr as $i => $Row) {
        ...
        if ($GroupBy == 'Month') {
            ...
            $Row['Name'] = $Row['Name'][1];
            $Row['Name'] = $Lang['MonthName'][(int) ($Row['Name'])] . ', ' . date('Y', $Row['Obj']->USTAMP);
        }
        if ($GroupBy == 'WeekDay') {
        ...
        if ($GroupBy == 'Order') {
            if ($Row['Obj']->CUSTOM_ORDER_ID) {
                $Row['Name'] .= ' ' . $Row['Obj']->CUSTOM_ORDER_ID;
            } else {
                $Row['Name'] .= ' ' . $Row['Obj']->ID;
            }
        ...
        if ($GroupBy == 'Flash') {
            if ($Row['Name'] == -1) {
                $Row['Obj']->NAME = $Row['Name'] = $Lang['None'];
            }
        }
        if ($GroupBy == 'Resolution') {
            if ($Row['Name'] == -1 || $Row['Name'] == 'x') {
                $Row['Obj']->NAME = $Row['Name'] = $Lang['Undefined'];
            }
        }
        if ($GroupBy == 'Pixel') {
            if ($Row['Name'] == -1) {
    ...
    if (!$Print) {
        include $nsTemplate->Inc('constructor/natural_form');
    }
...
function SortRows($a, $b)
{
    ...
    if ($OrderBy == 'ACTIONCNT') {
        $Key = 'CntAction';
    }
    if ($OrderBy == 'ACTIONUNI') {
        $Key = 'UniAction';
    }
    if ($OrderBy == 'SALECNT') {
        $Key = 'CntSale';
...
function QueriesLog($Query, $Time): void
{
    global $Db;
