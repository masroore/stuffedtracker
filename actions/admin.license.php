...
if (!$EditId && !$ViewId) {
    ...
    while ($Row = $Sql->Row()) {
        ...
        if (isset($Row->License['P']) && $Row->License['P'] == 2) {
            $Row->CLIENT = true;
        }
        if (isset($Row->License['P']) && $Row->License['P'] == 3) {
            $Row->AGENT = true;
        }
        if (!isset($Row->License['P'])) {
            $Row->ADD = true;
        }
...
function AddNewKey($Key)
{
    ...
    $KeyId = (int) (ValidVar($License['ID']));
    if (!ValidId($KeyId) || $KeyId == 0) {
        $Logs->Err($Lang['KeyIsInvalid']);

        return;
    }
    if (ValidVar($License['L']) != (int) (ValidVar($License['L']))) {
        $Logs->Err($Lang['KeyIsInvalid']);
...
function CheckRequirements($Req = false)
{
    ...
    for ($i = 0; $i < count($ReqArr); ++$i) {
        ...
        if (!isset($FullArr[$ReqValue[0]])) {
            return false;
        }  // если ключа P в общем массиве лицензии нет, то сразу false
        if (count($ReqValue) == 2) { // если не конкретизирован способ сравнения, то подразумевается equal
            for ($j = 0; $j < count($FullArr[$ReqValue[0]]); ++$j) {
                if (trim($FullArr[$ReqValue[0]][$j]) == $ReqValue[1]) {
                    $Return = false;
        ...
        if (count($ReqValue) == 3) { // если конкретизирован способ сравнения
            ...
            if (substr($ReqValue[1], 0, 3) == 'sum') {
                $Func = 'sum';
            }
            if (substr($ReqValue[1], 0, 3) == 'max') {
                $Func = 'max';
            }
            if (substr($ReqValue[1], 0, 3) == 'min') {
                $Func = 'min';
            }
            if ($Func == 'sum') {
                $Value = 0;
                for ($j = 0; $j < count($FullArr[$ReqValue[0]]); ++$j) {
                    $Value += trim($FullArr[$ReqValue[0]][$j]);
                }
            ...
            if ($Func == 'max') {
                $Value = 0;
                for ($j = 0; $j < count($FullArr[$ReqValue[0]]); ++$j) {
                    if ($Value < trim($FullArr[$ReqValue[0]][$j])) {
                        $Value = trim($FullArr[$ReqValue[0]][$j]);
                    }
                }
                $ReqValue[1] = substr($ReqValue[1], 3);
            ...
            if ($Func == 'min') {
                $Value = false;
                for ($j = 0; $j < count($FullArr[$ReqValue[0]]); ++$j) {
                    if ($Value !== false && $Value > trim($FullArr[$ReqValue[0]][$j])) {
                        $Value = trim($FullArr[$ReqValue[0]][$j]);
                    }
                }
                $ReqValue[1] = substr($ReqValue[1], 3);
            ...
            for ($j = 0; $j < count($FullArr[$ReqValue[0]]); ++$j) {
                ...
                if ($ReqValue[1] == '<=' && $Value <= $ReqValue[2]) {
                    $Return = false;
