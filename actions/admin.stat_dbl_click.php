...
$Sql = new Query($Query);
while ($Row = $Sql->Row()) {
    if ($Row->TIME_DBL_PAGE_LOAD == 0) {
        $Row->TIME_DBL_PAGE_LOAD = '';
    }
    if ($Row->TIME_DBL_ADV_CLICK == 0) {
        $Row->TIME_DBL_ADV_CLICK = '';
    }
    if ($Row->TIME_DBL_EVENT == 0) {
        $Row->TIME_DBL_EVENT = '';
    }
    if ($Row->TIME_DBL_SALE == 0) {
        $Row->TIME_DBL_SALE = '';
    }
...
if (!$CpId) {
    ...
    $Glob = $ST[$CpId][0];
    if ($Glob->STOP_DBL_PAGE_LOAD == 2) {
        $Glob->TIME_DBL_PAGE_LOAD = $ST[0][0]->TIME_DBL_PAGE_LOAD;
    }
    if ($Glob->STOP_DBL_ADV_CLICK == 2) {
        $Glob->TIME_DBL_ADV_CLICK = $ST[0][0]->TIME_DBL_ADV_CLICK;
    }
    if ($Glob->STOP_DBL_EVENT == 2) {
        $Glob->TIME_DBL_EVENT = $ST[0][0]->TIME_DBL_EVENT;
    }
    if ($Glob->STOP_DBL_SALE == 2) {
        $Glob->TIME_DBL_SALE = $ST[0][0]->TIME_DBL_SALE;
    }

    if ($nsProduct->LICENSE != 3) {
        if ($Glob->STOP_DBL_PAGE_LOAD == 2) {
            $Glob->STOP_DBL_PAGE_LOAD = $ST[0][0]->STOP_DBL_PAGE_LOAD;
        }
        if ($Glob->STOP_DBL_ADV_CLICK == 2) {
            $Glob->STOP_DBL_ADV_CLICK = $ST[0][0]->STOP_DBL_ADV_CLICK;
        }
        if ($Glob->STOP_DBL_EVENT == 2) {
            $Glob->STOP_DBL_EVENT = $ST[0][0]->STOP_DBL_EVENT;
        }
        if ($Glob->STOP_DBL_SALE == 2) {
            $Glob->STOP_DBL_SALE = $ST[0][0]->STOP_DBL_SALE;
        }
    }
    /*}
    if($nsProduct->LICENSE==3) {
