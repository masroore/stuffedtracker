...
if (!$GroupBy && !$NoGroupBy) {
    ...
    if ($GroupBy) {
        $nsProduct->Redir('paid_constructor', $Get . "&GroupBy=$GroupBy", 'report');
    }
}
...
if (ValidVar($Report->CampStat)) {
    ...
    if ($ShowListName) {
        $Report->CampStat[0]['Name'] = $ListName;
    }
    if (!$GroupBy && !$WhereArr) {
    ...
    if ($OrderBy == 'ACTCONV' || $OrderBy == 'SALECONV'
        ...
        if (count($Report->CampStat) > $Limit) {
            array_splice($Report->CampStat, $Limit);
        }
    ...
    if (!$Print) {
        include $nsTemplate->Inc('constructor/paid_form');
    }
