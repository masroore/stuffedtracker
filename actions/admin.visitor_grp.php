...
if (!ValidId($CurrentCompany->ID)) {
    $nsProduct->Redir('my_tracker', 'Mode=visitor_grps', 'admin');
}
...
if (ValidId($EditId)) {
    ...
    if (!ValidId($VisGrp->ID)) {
        $nsProduct->Redir('my_reports', '', 'admin');
    }
...
function CreateNewGrp($Arr): void
{
    ...
    if ($Watch) {
        SetVisWatch($NewId, $nsUser->UserId());
    }
...
function RemoveIp($IpId, $GrpId): void
{
    global $Db;
