...
if (ValidArr($EditArr)) {
    if (!ValidVar($EditArr['P3P'])) {
        $EditArr['P3P'] = '';
    }
    if (!ValidVar($EditArr['SSLink'])) {
        $EditArr['SSLink'] = '';
    }
    if (!ValidVar($EditArr['P3P_REF'])) {
        $EditArr['P3P_REF'] = '';
    }
    if (!ValidVar($EditArr['DefSkin'])) {
        $EditArr['DefSkin'] = '';
    }
    if (!ValidVar($EditArr['DefLang'])) {
        $EditArr['DefLang'] = '';
    }
    if (!ValidVar($EditArr['WhiteLogo'])) {
        $EditArr['WhiteLogo'] = '';
    }
    if (!ValidVar($EditArr['SendUsage'])) {
        $EditArr['SendUsage'] = 0;
    }
    if (!ValidVar($EditArr['UseStore'])) {
        $EditArr['UseStore'] = 0;
    }
    if (!ValidVar($EditArr['FromEmail'])) {
        $EditArr['FromEmail'] = '';
    }
    if (!ValidVar($EditArr['OnlinePeriod'])) {
        $EditArr['OnlinePeriod'] = 0;
    }
    if (!ValidVar($EditArr['UseIp'])) {
        $EditArr['UseIp'] = 0;
    }
    if (!ValidVar($EditArr['IpPeriod'])) {
        $EditArr['IpPeriod'] = 0;
    }
    if (!ValidVar($EditArr['IpNoCookie'])) {
        $EditArr['IpNoCookie'] = 0;
    }
    if (!ValidVar($EditArr['EnableFraud'])) {
        $EditArr['IpPeriod'] = 0;
    }
    if (!ValidVar($EditArr['FraudPeriod'])) {
        $EditArr['IpPeriod'] = 1;
    }
    if (!ValidVar($EditArr['FraudCount'])) {
        $EditArr['IpPeriod'] = 5;
    }
    if (!ValidVar($EditArr['UseWhiteLogo'])) {
        $EditArr['UseWhiteLogo'] = 0;
    }
    if (!ValidVar($EditArr['UseWhiteCopy'])) {
        $EditArr['UseWhiteCopy'] = 0;
    }
    if (!ValidVar($EditArr['TrafficPrior'])) {
        $EditArr['TrafficPrior'] = 'NONE';
    }
    if (!ValidVar($EditArr['PaidEntryPrior'])) {
        $EditArr['PaidEntryPrior'] = 'LAST';
    }
    if (!ValidVar($EditArr['NaturalEntryPrior'])) {
        $EditArr['NaturalEntryPrior'] = 'LAST';
    }
    if (!ValidVar($EditArr['NoneEntryPrior'])) {
        $EditArr['NoneEntryPrior'] = 'LAST';
    }

    if (!ValidVar($EditArr['VarCamp'])) {
        $EditArr['VarCamp'] = '';
    }
    if (!ValidVar($EditArr['VarCampSource'])) {
        $EditArr['VarCampSource'] = '';
    }
    if (!ValidVar($EditArr['VarKw'])) {
        $EditArr['VarKw'] = '';
    }
    if (!ValidVar($EditArr['VarKeyword'])) {
        $EditArr['VarKeyword'] = '';
    }
}
...
if (!ValidArr($EditArr)) {
    ...
    if (ValidVar($PriorArr[1])) {
        ...
        for ($i = 0; $i < count($EntryArr); ++$i) {
            ...
            if ($TmpArr[0] == 'NONE') {
                $EditArr['NoneEntryPrior'] = $TmpArr[1];
            }
            if ($TmpArr[0] == 'PAID') {
                $EditArr['PaidEntryPrior'] = $TmpArr[1];
            }
            if ($TmpArr[0] == 'NATURAL') {
                $EditArr['NaturalEntryPrior'] = $TmpArr[1];
            }
        }
    }
...
function SaveSettings($EditArr): void
{
...
function CheckStore()
{
    ...
    $Filename = 'test.txt';
    $f = @fopen(self . "/store/$Filename", 'a+b');
    if (!$f) {
        return false;
    }
    if (!@is_writable(self . "/store/$Filename")) {
        return false;
    }
...
function RebuildStoreFiles(): void
{
    $ActionFile = 'redir_action.nodb';
    $f = @fopen(self . "/store/$ActionFile", 'a+b');
    @flock($f, LOCK_EX);
    @ftruncate($f, 0);
    @flock($f, LOCK_UN);
    @fclose($f);
    ...
    $f = @fopen(self . "/store/$SplitFile", 'a+b');
    @flock($f, LOCK_EX);
    @ftruncate($f, 0);
    @flock($f, LOCK_UN);
    @fclose($f);
