...
if (!$nsUser->DEMO) {
    if ($Mode == 'reports' && ValidId($EditId) && ValidArr($EditArr)) {
        SaveUserReport($EditId, $EditArr);
    }
    if ($Mode == 'reports' && ValidId($DeleteId)) {
        DeleteUserReport($DeleteId);
    }
    if ($Mode == 'visitors' && ValidId($DeleteId)) {
        DeleteClientVisitor($DeleteId);
    }
    if ($Mode == 'visitor_grps' && ValidId($DeleteId)) {
        DeleteClientVisitorGrp($DeleteId);
    }

    if (ValidArr($AddToMy) && $Mode == 'reports') {
        AddToMy($AddToMy, 'REPORT_ID');
    }
    if (ValidId($MyDeleteId) && $Mode == 'reports') {
        DeleteFromMy($MyDeleteId, 'REPORT_ID');
    }
    if (ValidArr($AddToMy) && $Mode == 'visitors') {
        AddToMy($AddToMy, 'VISITOR_ID');
    }
    if (ValidId($MyDeleteId) && $Mode == 'visitors') {
        DeleteFromMy($MyDeleteId, 'VISITOR_ID');
    }
    if (ValidArr($AddToMy) && $Mode == 'visitor_grps') {
        AddToMy($AddToMy, 'VISITOR_GRP_ID');
    }
...
if ($Mode == 'list') {
    ...
    while ($Row = $Sql->Row()) {
        ...
        if ($Row->CONST_TYPE == 'NATURAL') {
            $Row->Addr = 'natural_constructor';
        }
        if ($Row->CONST_TYPE == 'PAID') {
            $Row->Addr = 'paid_constructor';
        }
        if ($Row->COMPANY_ID == 0) {
            $Row->CP_ID = $CurrentCompany->ID;
        } else {
    ...
    $UsersWhere = '';
    if (!$nsUser->ADMIN) {
        $UsersWhere = 'AND CV.COMPANY_ID=' . $CurrentCompany->ID;
    }
    $Query = '
		SELECT
    ...
    while ($Row = $Sql->Row()) {
        ...
        if ($Row->LAST_STAMP) {
            $Row->DATE_DIFF_NAME = LastStamp($Row->LAST_STAMP);
        }
    ...
    while ($Row = $Sql->Row()) {
        ...
        while ($SubRow = $SubSql->Row()) {
            if (strpos($SubRow->IP, '*') === false) {
                $GrpIpArr[] = "'" . $SubRow->IP . "'";
            } else {
                $IpTemplArr[] = $SubRow->IP;
            }
        }
    ...
    $ActionsWhere = '';
    if (!$nsUser->ADMIN) {
        $ActionsWhere = ' AND C.ID=' . $CurrentCompany->ID;
    }
    $Query = '
		SELECT
    ...
    $ActionItemsWhere = '';
    if (!$nsUser->ADMIN) {
        $ActionItemsWhere = ' AND AI.COMPANY_ID=' . $CurrentCompany->ID;
    }
    ...
    $SaleItemsWhere = '';
    if (!$nsUser->ADMIN) {
        $SaleItemsWhere = ' AND SI.COMPANY_ID=' . $CurrentCompany->ID;
    }
...
if ($Mode == 'visitors') {
    ...
    $VisitorsWhere = '';
    if (!$nsUser->ADMIN) {
        $VisitorsWhere = 'WHERE CV.COMPANY_ID=' . $CurrentCompany->ID;
    }
    $Query = '
		SELECT
...
if ($Mode == 'reports') {
    ...
    $ReportsWhere = '';
    if (!$nsUser->ADMIN) {
        $ReportsWhere = 'WHERE UR.COMPANY_ID=' . $nsUser->COMPANY_ID;
    }
    $Query = '
...
if ($Mode == 'actions') {
    ...
    $ActionsWhere = '';
    if (!$nsUser->ADMIN) {
        $ActionsWhere = 'WHERE S.COMPANY_ID=' . $CurrentCompany->ID;
    }
    $Query = '
		SELECT
...
if ($Mode == 'action_items') {
    ...
    $ActionItemsList = [];
    if (!$SelectCpId) {
        $SelectCpId = ValidVar($CurrentCompany->ID);
    }
    if ($nsUser->ADMIN) {
...
function SaveUserReport(&$Id, $Arr): void
{
    global $Db, $Logs, $Lang;
    extract($Arr);
...
function DeleteUserReport($Id): void
{
    global $Db, $nsProduct;
...
function AddToMy($Arr, $Field): void
{
    global $Db, $nsUser, $nsProduct;
...
function DeleteFromMy($Id, $Field): void
{
    global $Db, $nsUser, $nsProduct;
...
function LastStamp($Stamp)
{
    ...
    if ($DATE_DIFF == 0) {
        $DATE_DIFF_NAME = $Lang['Today'];
    }
    if ($DATE_DIFF == 1) {
        $DATE_DIFF_NAME = $Lang['Yesterday'];
    }
    if ($DATE_DIFF > 1) {
