...
for ($i = 0; $i < count($WhereArr); ++$i) {
    if (isset($WhereArr[$i]['Id'], $WhereArr[$i]['Name2'])) {
        if (isset($WhereArr[$i + 1])) {
            $ProgPath[$i + 1]['Url'] = getURL($Action, $WhereArr[$i + 1]['Url'], 'report');
        } else {
            $ProgPath[$i + 1]['Url'] = getURL($Action, $WhereArr[$i]['Url2'], 'report');
        }

        $ProgPath[$i + 1]['Name'] = '<b>' . $WhereArr[$i]['Name2'] . '</b> ';
        if (isset($WhereArr[$i + 1])) {
            $ProgPath[$i + 1]['Name'] .= '(' . $WhereArr[$i + 1]['Name'] . ')';
        }
    }
}
