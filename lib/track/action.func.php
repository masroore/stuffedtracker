...
{
    public function GetActionIds($PageId = false, $PathArr = false)
    {
        ...
        while ($Row = $Sql->Row()) {
            ...
            if (!$Row->PATH && self::CompareTemplate($Qr, $Row->QUERY)) {
                $Actions[$Row->ID]['Id'] = $Row->ID;
                if ($Item) {
                    $Actions[$Row->ID]['Item'] = urldecode($Item);
                }
            }
            if ($Row->PATH && self::ComparePathTemplate($CurrentPath, $Row->PATH, $Qr, $Row->QUERY)) {
                $Actions[$Row->ID]['Id'] = $Row->ID;
                if ($Item) {
                    $Actions[$Row->ID]['Item'] = urldecode($Item);
                }
    ...
    public function GetActionItemId($ItemName = false, $CompanyId = false)
    {
    ...
    public function ComparePathTemplate($CurrentPath = false, $Path = false, $Query = false, $Template = false)
    {
        if (self::CompareStrings($CurrentPath, $Path) && self::CompareTemplate($Query, $Template)) {
            return true;
        }
    ...
    public function CompareStrings($Str = false, $Templ = false)
    {
        global $_NS_TRACK_VARS;
        $Item = &$_NS_TRACK_VARS['Item'];
    ...
    public function CompareTemplate($Query = false, $Template = false)
    {
        $ItemValue = false;
        if (!$Template) {
            return true;
