...
class NS_TRACK_VISITOR
{
    public function GetVisitorId()
    {
        ...
        $AgentId = self::GetUserAgentId($UserAgent);
        $Visitor = false;
        $CookieId = false;
        $CountryId = 0;
        ...
        if (NS_TRACK_MISC::ValidVar($PresetID) && NS_TRACK_MISC::ValidId($PresetID)) {
            ...
            if (NS_TRACK_MISC::ValidId($Visitor->ID)) {
                $AgentId = ($Visitor->LAST_AGENT_ID) ?: $AgentId;
        ...
        if (!NS_TRACK_MISC::ValidVar($PresetID) || !NS_TRACK_MISC::ValidVar($Visitor->ID)) {
            ...
            if (!defined('NS_PHP_TRACKING')) {
                ...
                $Settings['All']->IP_TRACKING && $Settings['All']->IP_NO_COOKIE) {
                    $CookieId = self::TrackingVisitorCookie();
            ...
            if (!isset($_COOKIE[NS_COOKIE_PFX . 'visitor']) && $Settings['All']->IP_TRACKING) {
                $Visitor = self::FindByIp($IpId, $AgentId, $Settings['All']->IP_PERIOD);
                if ($Visitor) {
                    $CookieId = self::TrackingVisitorCookie($Visitor->COOKIE_ID);
                    self::FindLastNode($Visitor->ID);
                    $FindLastNode = false;
                }
            }
            if (!$CookieId) {
        ...
        if ($Visitor && NS_TRACK_MISC::ValidId($Visitor->ID)) {
            if (!$Visitor->FIRST_COUNTRY_ID) {
                $CountryId = self::GetCountryId($UserIp);
            } else {
            ...
            $Db->Query($Query);
            self::TrackingVisitorCookie($Visitor->COOKIE_ID);
    ...
    public function TrackingVisitorCookie($Cookie = false)
    {
        ...
        $Cookie = substr(md5(uniqid(mt_rand())), 0, 32);
        NS_TRACK_GENERAL::TrackingCookie(NS_COOKIE_PFX . 'visitor', $Cookie, time() + 60 * 60 * 24 * 10 * 365, '/', $Domain);

        return $Cookie;
    }

    public function GetUserAgentId($Agent = false)
    {
        if (!$Agent) {
            return 0;
        }
    ...
    public function FindByIp($IpId, $AgentId, $Days = 0)
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];

        $WhereArr[] = "LAST_IP_ID=$IpId";
        if (!$_NS_TRACK_VARS['PresetIP']) {
            $WhereArr[] = "LAST_AGENT_ID=$AgentId";
        }
    ...
    public function FindLastNode($VisId = 0, $AndSet = true)
    {
        global $_NS_TRACK_VARS, $_COOKIE;
        $Db = &$_NS_TRACK_VARS['Db'];
        ...
        if ($LogId) {
            NS_TRACK_GENERAL::SetCookieLog(false, $CheckId);
        }
    }
}
