...
{
    public function UpdateVisitorAction()
    {
    ...
    public function UpdateVisitorPath()
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
    ...
    public function UpdateVisitorSplit($SplitCamp): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $CompanyId = &$_NS_TRACK_VARS['CompanyId'];
    ...
    public function UpdateVisitorClick(): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $LogId = &$_NS_TRACK_VARS['LogId'];
    ...
    public function UpdateVisitorSale()
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $LogId = &$_NS_TRACK_VARS['LogId'];
        ...
        if ($CustomId && self::CheckDblSale($StId, $CustomId)) {
            return false;
        }
    ...
    public function UpdateStatUndef(): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $LogId = &$_NS_TRACK_VARS['LogId'];
    ...
    public function GetLogRecord()
    {
        ...
        if ($CookieStr) {
            ...
            if (NS_TRACK_MISC::ValidArr($CookieArr) && NS_TRACK_MISC::ValidVar($CookieArr[$StId])) {
                $CookieLog = $CookieArr[$StId];
            }
    ...
    public function GetCurrentSite($StId)
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $Site = false;
    ...
    public function GetSiteHosts($StId, $Site = false)
    {
        $Where = '';
        global $_NS_TRACK_VARS;
        $QueryClass = &$_NS_TRACK_VARS['QueryClass'];

        if ($Site && NS_TRACK_MISC::ValidId($Site->ID) && $Site->USE_HOSTS) {
            $Where = " AND ENABLED = '1'";
        }
        $Hosts = [];
    ...
    public function CurrentSiteHost($HostsArr = false, $PathArr = false, $Site = false)
    {
        $HostId = 0;
        if (!$PathArr || !$HostsArr) {
            return 0;
        ...
        if (!$HostId && $Site && !$Site->USE_HOSTS) {
            return self::AddNewSiteHost($Site->ID, $PathArr['host']);
        }

        return (NS_TRACK_MISC::ValidId($HostId)) ? $HostId : 0;
    }

    public function AddNewSiteHost($SiteId, $Host)
    {
        global $_NS_TRACK_VARS;
    ...
    public function PreparePathAddr($Addr)
    {
        $Arr = @parse_url(urldecode(urldecode($Addr)));
        if (!isset($Arr['path'])) {
            $Arr['path'] = '';
        }
        $Arr['path'] = preg_replace('#/+#D', '/', $Arr['path']);
        if ($Arr['path'] == '') {
            $Arr['path'] = '/';
        }
    ...
    public function SetCookieLog($StartNewNode = true, $SetLogId = false): void
    {
        ...
        self::TrackingCookie(NS_COOKIE_PFX . 'log', $CookieStr, time() + 60 * 60 * 24 * 10 * 365, '/', $Domain);
        $FindLastNode = false;
    ...
    public function FlushImg(): void
    {
        if (defined('NO_TRACK_IMG') && NO_TRACK_IMG) {
            return;
        }
    ...
    public function FinishTracking()
    {
        ...
        if (!defined('NS_PHP_TRACKING')) {
            ...
            return self::FlushImg();
        }

        return false;
    }

    public function TrackingCookie($Name = false, $Value = false, $Expire = false, $Path = false, $Domain = false, $Secure = false): void
    {
        if (defined('NS_PHP_TRACKING')) {
    ...
    public function CheckPathDblClick(): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $PageId = &$_NS_TRACK_VARS['PageId'];
	...
	";
        ...
        if (time() - $CheckStamp < $TimeLimit) {
            $Skip = true;
        }
    }

    public function CheckAdvDblClick(): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $VisId = &$_NS_TRACK_VARS['VisId'];
    ...
    public function CheckDblSale($StId, $CustomId)
    {
        if (!$CustomId) {
            return false;
        }
    ...
    public function CheckSaleDblClick(): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $VisId = &$_NS_TRACK_VARS['VisId'];
    ...
    public function CheckActionDblClick($ActionId): void
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $VisId = &$_NS_TRACK_VARS['VisId'];
    ...
    public function CheckClickFraud(&$Settings, $VisId = false, $SubId = false)
    {
        if (!$SubId || !$VisId) {
            return 0;
        }
        ...
        if ($Settings['All']->FRAUD_COUNT < 1) {
            return 0;
        }
        if ($Settings['All']->FRAUD_PERIOD < 1) {
            return 0;
        }
	...
	';
        ...
        if ($Count >= $Settings['All']->FRAUD_COUNT) {
            return 1;
        }
    }

    public function CheckTrackingMode()
    {
        ...
        if ($Mode == 'NONE' && $NonePoint == 'FIRST') {
            $NodeId = self::FindSomeNode('ANY', 'FIRST');
            if ($NodeId) {
                return self::UpdateCookieLog($NodeId);
            }

            return false;
        }

        $NodeId = self::FindSomeNode($Mode, 'FIRST');
        if ($NodeId) {
            return self::UpdateCookieLog($NodeId);
        }
        ...
        $NodeId = self::FindSomeNode((($Mode == 'PAID') ? 'NATURAL' : 'PAID'), (($Mode == 'PAID') ? $NaturalPoint : $PaidPoint));
        if ($NodeId) {
            return self::UpdateCookieLog($NodeId);
        }
    ...
    public function FindSomeNode($Mode = 'ANY', $Order = 'LAST')
    {
        global $_NS_TRACK_VARS;
        $Db = &$_NS_TRACK_VARS['Db'];
        $CompanyId = &$_NS_TRACK_VARS['CompanyId'];
    ...
    public function UpdateCookieLog($LogId)
    {
        self::SetCookieLog(false, $LogId);
