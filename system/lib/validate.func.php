...
function ReplaceSymb($Str, $File = false)
{
    if ($File === 1) {
        $Arr = explode('.', $Str);
        $Arr[0] = preg_replace('/[^[:alnum:]]/D', '', $Arr[0]);

        return $Arr[0] . '.' . $Arr[count($Arr) - 1];
    }

    return preg_replace('/[^[:alnum:]]/D', '', $Str);
}

function CheckSymb($Str)
{
    return preg_match('/[^[:alnum:]]/Di', $Str);
}
...
function ReplaceSymb_($Str, $File = false)
{
    if ($File === 1) {
        $Arr = explode('.', $Str);
        $Arr[0] = preg_replace('/[^[:alnum:]_]/D', '', $Arr[0]);

        return $Arr[0] . '.' . $Arr[count($Arr) - 1];
    }

    return preg_replace('/[^[:alnum:]]_/D', '', $Str);
}

function CheckSymb_($Str)
{
    return preg_match('/[^[:alnum:]_]/Di', $Str);
}
...
function FormError($Name = false, $Arr = false)
{
    ...
    if (!is_array($Arr) && isset($GLOBALS['ErrArr'])) {
        $Arr = &$GLOBALS['ErrArr'];
    }
    if (!is_array($Arr)) {
        return false;
    ...
    if (isset($Arr[$Name])) {
        echo '<br><span class=FormError>' . $Arr[$Name] . '</span>';
    }
}
