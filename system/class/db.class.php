...
class nsDatabase
{
    ...
    {
        global $Lang, $Logs;
        @mysql_close($this->ID) || $Logs->DbErr(false, $Lang['DbCloseErr']);
    }
    ...
    public function Select($Query = false, $Type = 'OBJ')
    {
        ...
        if (@mysql_error()) {
            $this->LastError = @mysql_error();
            $Logs->DbErr($Query, $Lang['DbFetchErr']);

            return;
        }
        if ($this->GetTime) {
            $this->ResTime = $this->GetMicroTime() - $StartTime;
        }
        if ($this->LoggerFunc) {
    ...
    public function TableExists($Table = false)
    {
        ...
        if (!is_array($this->TablesArr)) {
            $this->GetTables();
        }
        if (!in_array($Table, $this->TablesArr)) {
            return false;
        }
    ...
    public function CNT($Table = false, $Where = false)
    {
        if (!$Table) {
            return false;
        }
    ...
    public function IsExists($Table = false, $Field = false, $Value = false)
    {
    ...
    public function GenId($Table = false)
    {
        if (!$Table) {
            return false;
        }
...
class Query
{
    ...
    public function ReadSkinConfig(): void
    {
    ...
    public function Row()
    {
        ...
        if (isset($this->ListRowStyle) && is_array($this->ListRowStyle)) {
            if ($this->ColPos == count($this->ListRowStyle)) {
                $this->ColPos = 0;
            }
            $this->_STYLE = $this->ListRowStyle[$this->ColPos];
        } elseif (isset($this->ListRowColor) && is_array($this->ListRowColor)) {
            if ($this->ColPos == count($this->ListRowColor)) {
                $this->ColPos = 0;
            }
            $this->_COLOR = $this->ListRowColor[$this->ColPos];
        }

        if ($this->LastRec && $this->Position >= $this->LastRec) {
            return false;
        }

        if ($this->GetTime && $this->Position == -1) {
            $this->PrevTime = $this->GetMicroTime();
        }
        if ($this->GetTime) {
            $Prev = $this->PrevTime;
        ...
        if (@mysql_error()) {
            $Logs->DbErr($this->Query, $Lang['DbFetchErr']);

            return;
        }
        if ($this->AutoFree && !$Row) {
            $this->Free();
        }
    ...
    public function Seek($Pos = false)
    {
        if (!$this->QID) {
            return false;
        }
        ...
        if ($this->LastRec > -1 && $Pos > $this->LastRec) {
            return false;
        }
        ...
        if (@mysql_error()) {
            $Logs->DbErr($this->Query, $Lang['DbSeekErr']);
