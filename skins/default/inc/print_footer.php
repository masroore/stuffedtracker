...
<td bgcolor="<?php echo ((!$nsProduct->WHITE) ? '#707070' : '')?>" height=1>
<p><img src="<?php echo FileLink('images/0.gif'); ?>" width="1" height="1" border="0"></p>
</td>
</tr>
<tr>
...
<td>
<p class=Copyright><img src="<?php echo FileLink('images/0.gif'); ?>" width="20" height="1" border="0">
<?if ($nsProduct->LICENSE==1&&!$nsProduct->TRIAL_EXCEED) {
echo sprintf($Lang['TrialText'], $nsProduct->DAYS_LEFT, ParseNumberNow($nsProduct->DAYS_LEFT, $Lang['DayN'], $Lang['Day1'], $Lang['Day2']));
}?>
<?if ($nsProduct->TRIAL_EXCEED) echo $Lang['TrialExpired'];?>

<?if (!$nsProduct->WHITE || !$nsProduct->WHITE_NO_COPY) {?>
<?php echo str_replace('{VERSION}', $nsProduct->VERSION, $Lang['Powered'])?>
<?}?>
</p>
</td>
</tr>
