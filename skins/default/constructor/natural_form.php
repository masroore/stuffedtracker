...
<?GetFORM(false, false, false, "ID=\"ConstructorForm\"");?>
...
<tr ID="Filter1"  style="<?=(($ShowFilter)?"":"display:none")?>"><td class=FormSimpleTd>
	...
	<select name=ShowAll <?=(($DisableRefFilter)?"disabled":"")?> style="width:220px">
	<option value=0 <?=((!$ShowAll))?"selected":""?>><?=$Lang['OnlyRef']?></option>
	<option value=1 <?=(($ShowAll==1))?"selected":""?>><?=$Lang['NotOnlyRef']?></option>
	<option value=2 <?=(($ShowAll==2))?"selected":""?>><?=$Lang['OnlyNotRef']?></option>
	</select>
	...
	<td width=33%  align=right>
		...
		<label for="OnlyNatural"><?=$Lang['NaturalOnly']?></label>
		</td><td><input type=checkbox id="OnlyNatural" value=1 name=OnlyNatural <?=(($OnlyNatural==1)?"checked":"")?>></td></tr>
		</table>
	</td>

	</tr>
	<tr><td colspan=5 height=5><img src="<?=FileLink("images/0.gif");?>" width="1" height="5" border="0"></td></tr>
	...
	<td width=33% align=right>
	<?=$Lang['Limit']?>:&nbsp;
	<input type=text size=5 class=DateFld name=Limit value="<?=$Limit?>">
	</td>
	</tr>
